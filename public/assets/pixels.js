...
(function () {
  ...
  var mm =
    (window.MARKETS &&
      market &&
      window.MARKETS[market] &&
      window.MARKETS[market].pixels) ||
    {};
  ...
  if (ttId) {
    !(function (w, d, t) {
      ...
      ttq.setAndDefer = function (e, n) {
        e[n] = function () {
          e.push([n].concat([].slice.call(arguments, 0)));
        };
      };
  ...
  function fbStandard(name, params, evId) {
  ...
  function fbCustom(name, params, evId) {
    if (!window.fbq) return;

    var opts = evId ? { eventID: evId } : undefined;
    window.fbq("trackCustom", name, params || {}, opts);
  }

  window.SocioPixels = {
    pageView: function () {
      fbStandard("PageView", { content_category: market });
    },
    viewContent: function () {
      fbStandard("ViewContent", { content_category: market });
    },
    funnelStart: function () {
      fbStandard("InitiateCheckout", { content_category: market });
      if (window.ttq) ttq.track("ClickButton", { content_type: market });
    },
    lead: function (m, evId, leadType) {
      fbStandard(
        "Lead",
        { content_category: m || market, lead_type: leadType || undefined },
        evId,
      );
      if (window.ttq) ttq.track("SubmitForm", { content_type: m || market });
    },
    leadQualified: function (m, evId) {
      fbCustom("LeadCalificado", { content_category: m || market }, evId);
    },
    leadA: function (m, evId) {
      fbCustom("LeadA", { content_category: m || market }, evId);
    },
    schedule: function (m) {
      fbStandard("Schedule", { content_category: m || market });
    },
    contact: function (m) {
      fbStandard("Contact", { content_category: m || market });
      if (window.ttq) ttq.track("Contact", { content_type: m || market });
    },
    referral: function (m) {
      fbStandard("CompleteRegistration", {
        content_category: m || market,
        status: "referral",
      });
    },
    debug: function () {
      return { market: market, facebook: !!fbId, tiktok: !!ttId };
    },
  };
})();
