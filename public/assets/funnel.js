...
(function () {
  ...
  var state = { contact: {}, marketAccess: {}, qualification: {}, finalAsk: {}, hasAccess: null };
  ...
  function saveLead(stage, patch) {
    return fetch('/api/lead', {
      ...
      body: JSON.stringify({ sessionId: sessionId, market: marketKey, stageReached: stage, patch: patch })
    }).catch(function () {});
  }
  ...
  function setProgress(p) { bar.style.width = Math.max(4, Math.min(100, p)) + '%'; }
  function render(node) { app.innerHTML = ''; app.appendChild(node); window.scrollTo(0, 0); }
  function esc(s) { return String(s == null ? '' : s).replace(/[<>&"]/g, function (c) { return ({ '<': '&lt;', '>': '&gt;', '&': '&amp;', '"': '&quot;' })[c]; }); }
  function sealHTML() {
    return '<span class="seal"><span class="dot"></span>origen verificable' +
      '<svg viewBox="0 0 24 24"><path class="chk" d="M5 13l4 4L19 7"/></svg></span>';
  }
  function backBtn(fn) { var b = el('<button class="back">&larr; Atras</button>'); b.onclick = fn; return b; }
  ...
  function stepHero() {
    ...
    var node = el(
      '<div class="step">' +
        '<p class="eyebrow">' + esc(M.eyebrow) + '</p>' + dis +
        '<h1 class="title">' + esc(M.hero.title) + '</h1>' +
        '<p class="sub">' + esc(M.hero.sub) + '</p>' +
        '<div style="margin:1.2rem 0 1.6rem">' + sealHTML() + '</div>' +
  ...
  function stepTriage() {
    setProgress(18);
    var opts = M.triage.options.map(function (o, i) {
      return '<button class="opt" data-i="' + i + '"><span>' + esc(o.label) + '</span><span class="tick"></span></button>';
    }).join('');
    var node = el(
      '<div class="step"><p class="eyebrow">' + esc(M.eyebrow) + '</p>' +
        '<h2 class="q">' + esc(M.triage.question) + '</h2>' +
    ...
    node.querySelectorAll('.opt').forEach(function (b) {
      b.onclick = function () {
        ...
        if (state.hasAccess) stepContact(); else stepContactB();
      };
    });
  ...
  function stepContact() {
    ...
    node.querySelector('#next').onclick = function () {
      ...
      if (!name || (!email && !wa)) { alert('Indica tu nombre y al menos un medio de contacto.'); return; }
      state.contact = { name: name, email: email, whatsapp: wa,
        company: node.querySelector('#company').value.trim(), role: node.querySelector('#role').value.trim() };
      saveLead(2, { contact: state.contact, marketAccess: state.marketAccess, campaign: campaign });
      stepProof();
    };
  ...
  function stepProof() {
    setProgress(48);
    var node = el(
      '<div class="step"><h2 class="q">' + esc(M.proof.intro) + '</h2>' +
        '<div class="field"><label>' + esc(M.proof.actorLabel) + '</label>' +
        '<textarea id="actor" placeholder="' + esc(M.proof.actorPlaceholder) + '"></textarea></div>' +
        '<div class="row2"><div class="field"><label>Anos en el sector</label><input id="years" inputmode="numeric" placeholder="Ej. 5"></div>' +
    ...
    node.querySelectorAll('#intro .opt').forEach(function (b) {
      b.onclick = function () {
        ...
        state.marketAccess.actorNamed = node.querySelector('#actor').value.trim();
        state.marketAccess.experienceYears = parseInt(node.querySelector('#years').value, 10) || 0;
        state.marketAccess.proofText = node.querySelector('#actor').value.trim();
        state.marketAccess.proofFileUrl = node.querySelector('#link').value.trim();
  ...
  function stepQualification(i) {
    ...
    if (i >= qns.length) { saveLead(3, { qualification: state.qualification }); return stepFinal(); }
    setProgress(48 + Math.round((i + 1) / (qns.length + 1) * 32));
    var item = qns[i];
    var opts = item.options.map(function (o) {
      return '<button class="opt" data-v="' + o[1] + '"><span>' + esc(o[0]) + '</span><span class="tick"></span></button>';
    }).join('');
    var node = el(
      '<div class="step"><p class="eyebrow">Calificacion ' + (i + 1) + '/' + qns.length + '</p>' +
        '<h2 class="q">' + esc(item.q) + '</h2><div class="options">' + opts + '</div></div>'
    );
    node.querySelectorAll('.opt').forEach(function (b) {
      b.onclick = function () { state.qualification[item.id] = b.dataset.v; stepQualification(i + 1); };
    });
    node.appendChild(backBtn(function () { i === 0 ? stepProof() : stepQualification(i - 1); }));
    render(node);
  }
  ...
  function stepFinal() {
    setProgress(86);
    var fa = M.finalAsk;
    var opts = fa.options.map(function (o) {
      return '<button class="opt" data-v="' + o[1] + '"><span>' + esc(o[0]) + '</span><span class="tick"></span></button>';
    }).join('');
    var node = el('<div class="step"><h2 class="q">' + esc(fa.q) + '</h2><div class="options">' + opts + '</div></div>');
    node.querySelectorAll('.opt').forEach(function (b) {
      b.onclick = function () {
  ...
  function stepThanksA(choice) {
    ...
    var node = el(
      '<div class="step"><div style="margin-bottom:1.2rem">' + sealHTML() + '</div>' +
    ...
    node.querySelector('#ref').onclick = function () { stepReferral(true); };
    render(node);
  }
  ...
  function stepContactB() {
    ...
    node.querySelector('#next').onclick = function () {
      ...
      if (!name || (!email && !wa)) { alert('Indica tu nombre y un medio de contacto.'); return; }
      state.contact = { name: name, email: email, whatsapp: wa };
  ...
  function stepReferral(fromAccessPath) {
    ...
    node.querySelector('#send').onclick = function () {
      ...
      if (!pname) { alert('Indica al menos el nombre de la persona.'); return; }
      var payload = {
        market: marketKey, referrerLeadId: sessionId,
        referrerContact: state.contact,
        referred: { name: pname, contact: node.querySelector('#pcontact').value.trim(),
    ...
    node.appendChild(backBtn(fromAccessPath ? function () { stepThanksA('x'); } : stepContactB));
    render(node);
  }
  ...
  function stepThanksB(code) {
    ...
    var couponBlock = code
      ? '<p class="muted">Tu cupo de curso:</p><div class="coupon">' + esc(code) + '</div>'
      : '<p class="sub">Gracias. Te contactaremos con tu cupo de curso.</p>';
    var node = el(
      '<div class="step"><div style="margin-bottom:1.2rem">' + sealHTML() + '</div>' +
        '<h1 class="title">Gracias por la recomendacion.</h1>' + couponBlock +
        '<button class="btn ghost full" id="more" style="margin-top:1rem">Referir a otra persona</button></div>'
    );
    node.querySelector('#more').onclick = function () { stepReferral(false); };
    render(node);
  }
